...
void DepthDerivativePass::createRenderTextures(
    ...
    HeapData* srvHeap)
{
    mpRenderTexture = createRenderTexture(mpDevice, rtvHeap, srvHeap, size, DXGI_FORMAT_R32G32_FLOAT);
}
...
void DepthDerivativePass::forward(RenderContext* pRenderContext, RenderData& renderData)
{
    ...
    renderData.outputGPUHandleDictionary["gDepthDerivative"] = mpRenderTexture->getGPUSrvHandler();
    return;
}