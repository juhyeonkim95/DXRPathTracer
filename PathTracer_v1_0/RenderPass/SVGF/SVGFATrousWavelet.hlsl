...
//
//            const bool outside = (ipos2.x < 0) || (ipos2.x >= 720) || (ipos2.y < 0) || (ipos2.y >= 1280);
//
...
//                w = 1.0f;
//                float weight = 1.0f;// kernelWeights[abs(offsety)] * kernelWeights[abs(offsetx)] * w;
//
//                c += weight * qColor;
...
float4 main(VS_OUTPUT input) : SV_TARGET
{
    ...
    for (int offsetx = -support; offsetx <= support; offsetx++) {
        for (int offsety = -support; offsety <= support; offsety++) {
            const int2 ipos2 = ipos + int2(offsetx, offsety) * step;

            const bool outside = (ipos2.x < 0) || (ipos2.x >= 1024) || (ipos2.y < 0) || (ipos2.y >= 1024);

            if (!outside) {
                float3 qColor = gColorVariance.Load(int3(ipos2, 0)).rgb;
                float weight = 1.0f;// kernelWeights[abs(offsetx)] * kernelWeights[abs(offsety)];
                c += weight * qColor;
                weights += weight;
            }
        }
