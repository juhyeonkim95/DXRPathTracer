...
AccelerationStructureBuffers createBottomLevelAS(ID3D12Device5Ptr pDevice, ID3D12GraphicsCommandList4Ptr pCmdList, Mesh& mesh)
{
    ...
    D3D12_RAYTRACING_GEOMETRY_DESC geomDesc = {};
    geomDesc.Type = D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES;
    geomDesc.Triangles.VertexBuffer.StartAddress = mesh.getVerticesBuffer()->GetGPUVirtualAddress();
    geomDesc.Triangles.VertexBuffer.StrideInBytes = sizeof(vec3);
    geomDesc.Triangles.VertexFormat = DXGI_FORMAT_R32G32B32_FLOAT;
    geomDesc.Triangles.VertexCount = mesh.getVerticesNumber();

    geomDesc.Triangles.IndexBuffer = mesh.getIndicesBuffer()->GetGPUVirtualAddress();
    geomDesc.Triangles.IndexCount = mesh.getIndicesNumber();
...
AccelerationStructureBuffers createTopLevelAS(ID3D12Device5Ptr pDevice, ID3D12GraphicsCommandList4Ptr pCmdList, std::vector<ID3D12ResourcePtr> &pBottomLevelAS, uint64_t& tlasSize)
{
    size_t numBottomLevelAS = pBottomLevelAS.size();
    ...
    buffers.pInstanceDesc = createBuffer(pDevice, sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * numBottomLevelAS, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_GENERIC_READ, kUploadHeapProps);
    ...
    for (uint32_t i = 0; i < numBottomLevelAS; i++) {
        ...
        instanceDescs[i].AccelerationStructure = pBottomLevelAS[i]->GetGPUVirtualAddress();
